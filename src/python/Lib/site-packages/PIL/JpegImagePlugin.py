...
from .Jpegconfigs import configs
...
def _save(im: Image.Image, fp: IO[bytes], filename: str | bytes) -> None:
    ...
    if quality == "keep":
        ...
        subsampling = "keep"
        qtables = "keep"
    elif quality in configs:
        config = configs[quality]
        quality = -1
        subsampling = config.get("subsampling", -1)
        qtables = config.get("quantization")
    elif not isinstance(quality, int):
        msg = "Invalid quality setting"
        raise ValueError(msg)
    else:
        if subsampling in configs:
            subsampling = configs[subsampling].get("subsampling", -1)
        if isinstance(qtables, str) and qtables in configs:
            qtables = configs[qtables].get("quantization")
